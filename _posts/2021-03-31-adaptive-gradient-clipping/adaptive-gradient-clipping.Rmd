...
To summarize, the `Weight standardization` and `Activation Scaling` in combination affect the mean-shift that `BatchNorm` provides. The `Adaptive Gradient Clipping` helps maintain this activation mean by making sure the parameters don't significantly change that disrupts this mean.  
...
There are interesting future avenues using these tricks. Specifically, in terms of Meta Learning for classification, how the pre-training gradients effect and translate to task-specific gradients. 
